            ...
            */string jsonData = @"[
                    {
                        ...
                        ""ruzgar"":""23km/sa"",
                        ""status"":""Hava-Durumu-Tahmini""
                    }
                ]";

            // JSON verilerini bir nesne koleksiyonuna dönüştürün.
            var result = System.Text.Json.JsonSerializer.Deserialize<List<HAVADURUMU>>(jsonData);

            // Nesne koleksiyonunu yazdırın.
            foreach (var HAVA in result)
            {
                AddHAVADURUMUCommand command = new AddHAVADURUMUCommand();
                command.HAVA = HAVA;
                
                AddHAVADURUMUCommand.Handler handler = new AddHAVADURUMUCommand.Handler();
                var data = await handler.Handle(command,CancellationToken.None);
            };